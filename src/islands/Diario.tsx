...
import { diarioNavegador, diasConPractica, type Entrada } from '../lib/diario';
...
function Cuerpo({ e, d, recargar }: { e: Entrada; d: D; recargar: () => void }) {
  const x = e.datos as Record<string, string & string[]>;
  const [borrador, setBorrador] = useState('');
  switch (e.tipo) {
    case 'reflexion':
      return (
        <>
          <p class="preg">{x.pregunta}</p>
          {x.respuesta ? <p>{x.respuesta}</p> : (
            <form onSubmit={async (ev) => { ev.preventDefault(); await d.actualizar(e.id, { respuesta: borrador }); recargar(); }}>
    ...
    case 'circulo':
      return <><p><strong>Depende de mí:</strong> {x.dentro?.join(' · ')}</p><p><strong>No depende:</strong> {x.fuera?.join(' · ')}</p>{x.paso && <p><strong>Primer paso:</strong> {x.paso}</p>}</>;
    case 'termometro':
      return <><p><strong>Hecho:</strong> {x.hecho}</p><p><strong>Juicio:</strong> {x.juicio} <em>({x.cierto})</em></p>{x.depende && <p><strong>Depende de mí:</strong> {x.depende}</p>}</>;
    case 'examen':
      return <><p><strong>Bien:</strong> {x.bien}</p><p><strong>Falla:</strong> {x.fallo}</p><p><strong>Mañana:</strong> {x.manana}</p></>;
  }
}
